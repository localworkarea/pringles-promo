...
export function addTouchAttr() {
	// Додавання data-att-touch для HTML, якщо браузер мобільний
	if (isMobile.any()) document.documentElement.setAttribute('data-att-touch', '')
}
// Додавання loaded для HTML після повного завантаження сторінки
export function addLoadedAttr() {
	if (!document.documentElement.hasAttribute('data-att-preloader-loading')) {
...
export let bodyLockToggle = (delay = 500) => {
	if (document.documentElement.hasAttribute("data-att-scrolllock")) {
		bodyUnlock(delay)
	} else {
		bodyLock(delay)
	}
