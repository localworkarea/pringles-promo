...
class FullPage {
	constructor(element, options) {
		let config = {
			...
			// Кут свайпа: секція перемикається лише якщо рух переважно вертикальний.
			// |yCoord| має бути більшим за |xCoord| * swipeAngleRatio.
			// 1.0 ≈ 45°, 1.2 ≈ 50°, 1.5 ≈ 56°, 1.73 ≈ 60° (більше — суворіше)
			swipeAngleRatio: 1.2,
			//===============================
			// Ефекти
			// Ефекти: fade, cards, slider
			mode: element.dataset.attFullpageEffect ? element.dataset.attFullpageEffect : 'slider',
			//===============================
			// Булети
			// Активація буллетів
			bullets: element.hasAttribute('data-bullets') ? true : false,
	...
	setClasses() {
		...
		for (let index = 0; index < this.sections.length; index++) {
			document.documentElement.classList.remove(`--section-${index}`);
		}
		document.documentElement.classList.add(`--section-${this.activeSectionId}`);
	...
	setEvents() {
		...
		// работаем с переключением слайдов по клику на кнопку/кнопки [data-fp-switch-btn]
	...
	touchDown(e) {
		...
		this._xP = e.changedTouches[0].pageX;
	...
	touchMove(e) {
		...
		// Перевірка кута свайпа: перемикаємо секцію лише при переважно вертикальному русі
		const isVertical = Math.abs(yCoord) > Math.abs(xCoord) * this.options.swipeAngleRatio;
		// Чи дозволено перехід?
		this.checkScroll(yCoord, targetElement);
		// Перехід
		if (this.goScroll && isVertical && Math.abs(yCoord) > 20) {
	...
	switchingSection(idSection = this.activeSectionId, direction) {
		...
		if (this.stopEvent) {
			...
			let delaySection;
			if (direction < 0) {
				delaySection = this.activeSection.dataset.attFullpageDirectionUp ? parseInt(this.activeSection.dataset.attFullpageDirectionUp) : 500;
				document.documentElement.classList.add('--scroll-up');
				document.documentElement.classList.remove('--scroll-down');
			} else {
				delaySection = this.activeSection.dataset.attFullpageDirectionDown ? parseInt(this.activeSection.dataset.attFullpageDirectionDown) : 500;
...
if (document.querySelector('[data-att-fullpage]')) {
	window.addEventListener('load', () => window.flsFullpage = new FullPage(document.querySelector('[data-att-fullpage]')))
}
