...
class BeforeAfter {
	constructor(props) {
		...
		if (this.config.init) {
			const beforeAfterItems = document.querySelectorAll('[data-att-beforeafter]');
	...
	beforeAfterClasses(beforeAfter) {
		const beforeAfterArrow = beforeAfter.querySelector('[data-att-beforeafter-arrow]');
		beforeAfter.addEventListener('mouseover', function (e) {
			const targetElement = e.target;
			if (!targetElement.hasAttribute('data-att-beforeafter-arrow')) {
				if (targetElement.closest('[data-att-beforeafter-before]')) {
	...
	beforeAfterItemInit(beforeAfter) {
		const beforeAfterArrow = beforeAfter.querySelector('[data-att-beforeafter-arrow]');
		const afterItem = beforeAfter.querySelector('[data-att-beforeafter-after]');
