...
function zoomInit() {
	const zoomItems = document.querySelectorAll('[data-att-zoom]')
	...
	function inAction(e) {
		...
		if (targetElement.closest('[data-att-zoom]')) {
			const zoomBlock = targetElement.closest('[data-att-zoom]')
			const zoomValue = +zoomBlock.dataset.attZoom || 3
			...
			if (!zoomImage) {
				...
				if (e.type === 'mouseover') {
					...
					cloneImage.setAttribute('data-att-zoom-image', '')
					cloneImage.src = zoomImage.src
					if (!zoomImageParent.querySelector('[data-att-zoom-image]')) {
					...
					setStyles(zoomImageParent.querySelector('[data-att-zoom-image]'), zoomImage, mousePos, zoomValue)
				} else if (e.type === 'mouseout') {
					if (zoomImageParent.querySelector('[data-att-zoom-image]')) {
						zoomImageParent.querySelector('[data-att-zoom-image]').remove()
					}
				}
			}
		}
...
document.querySelector('[data-att-zoom]') ?
