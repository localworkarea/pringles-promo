...
const marquee = () => {
	const $marqueeArray = document.querySelectorAll("[data-att-marquee]");
	const ATTR_NAMES = {
		wrapper: "data-att-marquee-wrapper",
		inner: "data-att-marquee-inner",
		item: "data-att-marquee-item",
	};
	...
	$marqueeArray.forEach(($wrapper) => {
		...
		const dataMarqueeSpace = parseFloat($wrapper.getAttribute("data-att-marquee-space"));
		const $items = $wrapper.querySelectorAll(`[${ATTR_NAMES.item}]`);
		const speed = parseFloat($wrapper.getAttribute("data-att-marquee-speed")) / 10 || 100;
		const isMousePaused = $wrapper.hasAttribute("data-att-marquee-pause");
		const direction = $wrapper.getAttribute("data-att-marquee-direction");
		...
		let startPosition = parseFloat($wrapper.getAttribute("data-att-marquee-start")) || 0;
