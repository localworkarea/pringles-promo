...
class SelectConstructor {
	constructor(props, data = null) {
		...
		if (this.config.init) {
			...
			const selectItems = data ? document.querySelectorAll(data) : document.querySelectorAll('select[data-att-select]');
	...
	selectInit(originalSelect, index) {
		...
		index ? originalSelect.dataset.attSelectId = index : null;
		...
		if (originalSelect.options.length) {
			...
			originalSelect.dataset.attSelectSpeed = originalSelect.dataset.attSelectSpeed ? originalSelect.dataset.attSelectSpeed : this.config.speed;
			this.config.speed = +originalSelect.dataset.attSelectSpeed
	...
	selectBuild(originalSelect) {
		...
		// Додаємо ID селекту
		selectItem.dataset.attSelectId = originalSelect.dataset.attSelectId;
		// Отримуємо клас оригінального селекту, створюємо модифікатор та додаємо його
		originalSelect.dataset.attSelectModif ? selectItem.classList.add(`select--${originalSelect.dataset.attSelectModif}`) : null;
		// Якщо множинний вибір, додаємо клас
		originalSelect.multiple ? selectItem.classList.add(this.selectClasses.classSelectMultiple) : selectItem.classList.remove(this.selectClasses.classSelectMultiple);
		// Cтилізація елементів під checkbox (тільки для multiple)
		originalSelect.hasAttribute('data-att-select-checkbox') && originalSelect.multiple ? selectItem.classList.add(this.selectClasses.classSelectCheckBox) : selectItem.classList.remove(this.selectClasses.classSelectCheckBox);
		...
		originalSelect.hasAttribute('data-att-select-search') ? this.searchActions(selectItem) : null;
		// Якщо вказано налаштування data-open, відкриваємо селект
		originalSelect.hasAttribute('data-att-select-open') ? this.selectAction(selectItem) : null;
		// Обробник disabled
	...
	selectsActions(e) {
		...
		const selectItem = isSelect || document.querySelector(`.${this.selectClasses.classSelect}[data-att-select-id="${isTag.dataset.attSelectId}"]`);
		...
		if (type === 'click') {
			...
			if (tag) {
				const optionItem = document.querySelector(`.${this.selectClasses.classSelect}[data-att-select-id="${tag.dataset.attSelectId}"] .select__option[data-att-select-value="${tag.dataset.attSelectValue}"]`);
	...
	selectСlose(selectItem) {
		...
		if (!selectOptions.classList.contains('_slide')) {
			...
			slideUp(selectOptions, originalSelect.dataset.attSelectSpeed);
			setTimeout(() => {
				selectItem.style.zIndex = '';
			}, originalSelect.dataset.attSelectSpeed);
		}
	}
	...
	selectAction(selectItem) {
		...
		const selectOpenzIndex = originalSelect.dataset.attSelectZIndex ? originalSelect.dataset.attSelectZIndex : 3;
		...
		if (originalSelect.closest('[data-att-select-one]')) {
			const selectOneGroup = originalSelect.closest('[data-att-select-one]');
		...
		setTimeout(() => {
			if (!selectOptions.classList.contains('--slide')) {
				...
				slideToggle(selectOptions, originalSelect.dataset.attSelectSpeed);
	...
	getSelectTitleValue(selectItem, originalSelect) {
		...
		// Якщо увімкнено режим тегів (вказано налаштування data-att-select-tags)
		if (originalSelect.multiple && originalSelect.hasAttribute('data-att-select-tags')) {
			selectTitleValue = this.getSelectedOptionsData(originalSelect).elements.map(option => `<span role="button" data-att-select-id="${selectItem.dataset.attSelectId}" data-att-select-value="${option.value}" class="--select-tag">${this.getSelectElementContent(option)}</span>`).join('');
			// Якщо виведення тегів у зовнішній блок
			if (originalSelect.dataset.attSelectTags && document.querySelector(originalSelect.dataset.attSelectTags)) {
				document.querySelector(originalSelect.dataset.attSelectTags).innerHTML = selectTitleValue;
				if (originalSelect.hasAttribute('data-att-select-search')) selectTitleValue = false;
			}
		}
		// Значення або плейсхолдер
		selectTitleValue = selectTitleValue.length ? selectTitleValue : (originalSelect.dataset.attSelectPlaceholder || '')

		if (!originalSelect.hasAttribute('data-att-select-tags')) {
		...
		if (originalSelect.hasAttribute('data-att-select-pseudo-label')) {
			pseudoAttribute = originalSelect.dataset.attSelectPseudoLabel ? ` data-att-select-pseudo-label="${originalSelect.dataset.attSelectPseudoLabel}"` : ` data-att-select-pseudo-label="Заповніть атрибут"`;
			pseudoAttributeClass = ` ${this.selectClasses.classSelectPseudoLabel}`;
		}
		// Якщо є значення, додаємо клас
		this.getSelectedOptionsData(originalSelect).values.length ? selectItem.classList.add(this.selectClasses.classSelectActive) : selectItem.classList.remove(this.selectClasses.classSelectActive);
		// Повертаємо поле введення для пошуку чи текст
		if (originalSelect.hasAttribute('data-att-select-search')) {
			// Виводимо поле введення для пошуку
			return `<div class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-att-select-placeholder="${selectTitleValue}" class="${this.selectClasses.classSelectInput}"></span></div>`;
		} else {
			// Якщо вибрано елемент зі своїм класом
			const customClass = this.getSelectedOptionsData(originalSelect).elements.length && this.getSelectedOptionsData(originalSelect).elements[0].dataset.attSelectClass ? ` ${this.getSelectedOptionsData(originalSelect).elements[0].dataset.attSelectClass}` : '';
	...
	getSelectElementContent(selectOption) {
		...
		const selectOptionData = selectOption.dataset.attSelectAsset ? `${selectOption.dataset.attSelectAsset}` : '';
	...
	getSelectPlaceholder(originalSelect) {
		...
		if (selectPlaceholder) {
			return {
				...
				show: selectPlaceholder.hasAttribute("data-att-select-show"),
				label: {
					show: selectPlaceholder.hasAttribute("data-att-select-label"),
					text: selectPlaceholder.dataset.attSelectLabel
				}
			}
		}
	}
	...
	getOptions(originalSelect) {
		...
		const selectOptionsScroll = originalSelect.hasAttribute('data-att-select-scroll') ? `` : '';
		const customMaxHeightValue = +originalSelect.dataset.attSelectScroll ? +originalSelect.dataset.attSelectScroll : null;
		// Отримуємо елементи списку
		let selectOptions = Array.from(originalSelect.options);
		if (selectOptions.length > 0) {
			let selectOptionsHTML = ``;
			// Якщо вказано налаштування data-att-select-show, показуємо плейсхолдер у списку
	...
	getOption(selectOption, originalSelect) {
		...
		// Якщо елемент вибраний і немає налаштування data-att-select-show-selected, приховуємо елемент
		const selectOptionHide = selectOption.selected && !originalSelect.hasAttribute('data-att-select-show-selected') && !originalSelect.multiple ? `hidden` : ``;
		// Якщо для елемента зазначений клас додаємо
		const selectOptionClass = selectOption.dataset.attSelectClass ? ` ${selectOption.dataset.attSelectClass}` : '';
		// Якщо вказано режим посилання
		const selectOptionLink = selectOption.dataset.attSelectHref ? selectOption.dataset.attSelectHref : false;
		const selectOptionLinkTarget = selectOption.hasAttribute('data-att-select-href-blank') ? `target="_blank"` : '';
		// Будуємо та повертаємо конструкцію елемента
		let selectOptionHTML = ``;
		selectOptionHTML += selectOptionLink ? `<a ${selectOptionLinkTarget} ${selectOptionHide} href="${selectOptionLink}" data-att-select-value="${selectOption.value}" class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}">` : `<button ${selectOptionHide} class="${this.selectClasses.classSelectOption}${selectOptionClass}${selectOptionSelected}" data-att-select-value="${selectOption.value}" type="button">`;
	...
	setOptionsPosition(selectItem) {
		...
		const customMaxHeightValue = +originalSelect.dataset.attSelectScroll ? `${+originalSelect.dataset.attSelectScroll}px` : ``;
		const selectOptionsPosMargin = +originalSelect.dataset.attSelectOptionsMargin ? +originalSelect.dataset.attSelectOptionsMargin : 10;
	...
	optionAction(selectItem, originalSelect, optionItem) {
		...
		if (originalSelect.multiple) {
			...
			originalSelect.value = optionItem.dataset.attSelectValue || optionItem.textContent;
			this.selectAction(selectItem);
		}
		this.setSelectTitleValue(selectItem, originalSelect);
	...
	setSelectChange(originalSelect) {
		...
		if (originalSelect.hasAttribute('data-att-select-validate')) {
			formValidate.validateInput(originalSelect)
		}
		// При зміні селекту надсилаємо форму
		if (originalSelect.hasAttribute('data-att-select-submit') && originalSelect.value) {
...
document.querySelector('select[data-att-select]') ?
	window.addEventListener('load', () => window.attSelect = new SelectConstructor({})) : null
