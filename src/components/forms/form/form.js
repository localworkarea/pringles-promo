	...
	function formSubmit() {
		...
		if (forms.length) {
			for (const form of forms) {
				...
				!form.hasAttribute('data-att-form-novalidate') ? form.setAttribute('novalidate', true) : null
				// Подія відправки
		...
		async function formSubmitAction(form, e) {
			...
			if (error === 0) {
				if (form.dataset.attForm === 'ajax') { // Якщо режим ajax
					e.preventDefault();
				...
				} else if (form.dataset.attForm === 'dev') {	// Якщо режим розробки
					e.preventDefault()
					formSent(form)
				}
			} else {
	...
	function formFieldsInit() {
		document.body.addEventListener("focusin", function (e) {
			...
			if ((targetElement.tagName === 'INPUT' || targetElement.tagName === 'TEXTAREA')) {
				...
				targetElement.hasAttribute('data-att-form-validatenow') ? formValidate.removeError(targetElement) : null;
			}
		});
		document.body.addEventListener("focusout", function (e) {
			...
			if ((targetElement.tagName === 'INPUT' || targetElement.tagName === 'TEXTAREA')) {
				...
				// Миттєва валідація
				targetElement.hasAttribute('data-att-form-validatenow') ? formValidate.validateInput(targetElement) : null;
			}
		});
	}
	formSubmit()
